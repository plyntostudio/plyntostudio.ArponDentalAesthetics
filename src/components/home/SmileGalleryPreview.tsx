...
export function SmileGalleryPreview() {
  return (
    <section className="section-frame section-divider bg-highlight py-[4.5rem] lg:py-24" aria-label="Smile gallery preview">
      <div className="container-main">
        ...
        <div className="mt-12 grid grid-cols-2 gap-4 md:gap-6">
          {PLACEHOLDER_GRADIENTS.map((gradient, index) => (
            <div
              key={index}
              className={`image-surface aspect-square w-full bg-gradient-to-br ${gradient} transition-transform duration-300 hover:-translate-y-1`}
              aria-hidden="true"
            />
          ))}
        </div>
