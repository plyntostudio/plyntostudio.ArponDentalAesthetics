...
export function WhyChooseUsSection() {
  return (
    <section className="section-frame section-transition-top bg-highlight py-[4.5rem] lg:py-24" aria-label="Why choose us">
      <div className="container-main">
        ...
        <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {REASONS.map((reason) => (
            <div key={reason.title} className="panel-card px-6 py-7 transition-transform duration-200 hover:-translate-y-1">
              <div className="mb-5 h-[3px] w-12 rounded-full bg-accent" aria-hidden="true" />
              <h3 className="font-serif text-[1.34rem] leading-[1.15] tracking-[-0.02em] text-text-main">
                {reason.title}
              </h3>
